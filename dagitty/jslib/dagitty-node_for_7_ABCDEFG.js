...
const subsets = getAllSubsets(["A<->B", "A<->C", "A<->D", "A<->E", "A<->F", "A<->G", "B<->C", "B<->D", "B<->E", "B<->F", "B<->G", "C<->D", "C<->E", "C<->F", "C<->G","D<->E", "D<->F", "D<->G", "E<->F", "E<->G", "F<->G"]);
console.log('one!')

const jsubsets = subsets.map(x => x.join(' '))
console.log('two!!')

let pfad = ["A->B->C->D->E->F->G"]
...
const dags_out_7N = dags

// // write into file: 
// const fs = require('fs');

//////////////// <----------
const dagsPath = path.join(__dirname,'/flds_computed_data_folder_new_18_05_23/dags_out_7N')
fs.writeFile(dagsPath, JSON.stringify(dags_out_7N), function(err) {
    if(err) {
        return console.log(err);
    }
    console.log("The dags_out_7N file was saved!");
}); 
///////////////// <----------
...
fs.writeFile(monotonDagPath, JSON.stringify(subbydags), function(err) {
    ...
    console.log("The monoton_dags_7N file was saved!");
}); 
...
const not_mono_g = JSON.stringify(GraphAnalyzer.treeID(new Graph('dag { A->B->C->D->E->F->G A<->C }'))) 
console.log(not_mono_g)

...
// console.log(JSON.stringify(GraphAnalyzer.treeID(new Graph("dag { 1->2 2->3 }"))))
...
const subsets = getAllSubsets(["A<->B", "A<->C", "A<->D", "A<->E", "B<->C", "B<->D", "B<->E", "C<->D", "C<->E", "D<->E"]);
...
const data_out = dags.map(x => JSON.stringify(GraphAnalyzer.treeID(new Graph(x)))); 
// console.log(data_out)

// write into file: 
const fs = require('fs');

const dataPath = path.join(__dirname,'/flds_computed_data_folder/data_out')
fs.writeFile(dataPath, JSON.stringify(data_out), function(err) {
    if(err) {
        return console.log(err);
    }
    console.log("The data_out file was saved!");
}); 
...
const dagsPath = path.join(__dirname,'/flds_computed_data_folder/dags_out')
fs.writeFile(dagsPath, JSON.stringify(dags_out), function(err) {
    if(err) {
        return console.log(err);
    }
    console.log("The dags_out file was saved!");
}); 
...
const subbybase = subbys.map(x => x.map(y => (pfad + " " + y)))
const subbydags = subbybase.map(x => x.map(y => "dag { " + y + " }"))

// console.log(subbydags);

const monoton_alpha = subbydags.map(x => x.map(y => JSON.stringify(GraphAnalyzer.treeID(new Graph(y))))); 

// write into file: 
const monotonDagPath = path.join(__dirname,'/flds_computed_data_folder/monoton_dags')
...
fs.writeFile(monotonDagPath, JSON.stringify(subbydags), function(err) {
    ...
    console.log("The monoton_dags file was saved!");
}); 


// write into file: 
const monotonPath = path.join(__dirname,'/flds_computed_data_folder/monoton_data')

// const fs = require('fs');

fs.writeFile(monotonPath, JSON.stringify(monoton_alpha), function(err) {
    if(err) {
        return console.log(err);
    }
    console.log("The monoton_data file was saved!");
}); 
...
const not_mono_g = JSON.stringify(GraphAnalyzer.treeID(new Graph('dag { A->B->C->D->E A<->C }'))) 
console.log(not_mono_g)
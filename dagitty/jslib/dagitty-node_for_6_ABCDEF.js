...
// const _ = require("underscore") // <--- 'cannot find module underscore'
	...
	treeID : function (g) {
		...
		function fastpMightSatisfyQuadraticEquation(fastp, abc) {
			...
			for (var i=0;i<simulated.length;i++) {
				...
				if (i==0 && app_aqqs_bpr_bqst_crr_ctts < 0 && minus_2apq_bpt_bqr_2crt > 0) return false  // <--- added 'i==0 &&' :flds
				if (i==0 && app_aqqs_bpr_bqst_crr_ctts > 0 && minus_2apq_bpt_bqr_2crt < 0) return false // <--- added 'i==0 &&' :flds
...
//function from stackoverflow https://stackoverflow.com/questions/42773836/how-to-find-all-subsets-of-a-set-in-javascript-powerset-of-array
const getAllSubsets = 
...
const subsets = getAllSubsets(["A<->B", "A<->C", "A<->D", "A<->E", "A<->F", "B<->C", "B<->D", "B<->E", "B<->F", "C<->D", "C<->E", "C<->F","D<->E", "D<->F", "E<->F"]);
console.log('one!')

const jsubsets = subsets.map(x => x.join(' '))
console.log('two!!')

let pfad = ["A->B->C->D->E->F"]

const predags = jsubsets.map(x => (pfad + " " + x))
console.log('three!!!')
...
const data_out_6N = dags.map(x => {console.log(x); return JSON.stringify(GraphAnalyzer.treeID(new Graph(x)))} ); // <------
console.log('five!!!!!')

/////////////////////// <--------
// write into file: 
const fs = require('fs');
const dataPath = path.join(__dirname,'/flds_computed_data_folder/data_out_6N')
fs.writeFile(dataPath, JSON.stringify(data_out_6N), function(err) {
    if(err) {
        return console.log(err);
    }
    console.log("The data_out_6N file was saved!");
}); 
////////////////////////
...
const dags_out_6N = dags

// // write into file: 
// const fs = require('fs');

//////////////// <----------
const dagsPath = path.join(__dirname,'/flds_computed_data_folder/dags_out_6N')
fs.writeFile(dagsPath, JSON.stringify(dags_out_6N), function(err) {
    if(err) {
        return console.log(err);
    }
    console.log("The dags_out_6N file was saved!");
}); 
///////////////// <----------
...
// console.log(subsubsetright[testnum])
// console.log(subsets[testnum].length)
// console.log(subsets[testnum])

// console.log(subsubsetright)


const subbys = subsubsetright.map(x => x.map(y => y.join(' ')))

let peath = ["A->B->C->D->E->F"]
...
const monoton_data_6N = subbydags.map(x => x.map(y => JSON.stringify(GraphAnalyzer.treeID(new Graph(y))))); 

// write into file: 
const monotonDagPath = path.join(__dirname,'/flds_computed_data_folder/monoton_dags_6N')

// const fs = require('fs');

fs.writeFile(monotonDagPath, JSON.stringify(subbydags), function(err) {
    if(err) {
        return console.log(err);
    }
    console.log("The monoton_dags_6N file was saved!");
}); 


// write into file: 
const monotonPath = path.join(__dirname,'/flds_computed_data_folder/monoton_data_6N')

// const fs = require('fs');

fs.writeFile(monotonPath, JSON.stringify(monoton_data_6N), function(err) {
    if(err) {
        return console.log(err);
    }
    console.log("The monoton_data_6N file was saved!");
}); 


// const not_mono_g = JSON.stringify(GraphAnalyzer.treeID(new Graph('dag { A->B->C->D->E A<->C B<->C B<->D C<->D }'))) 
// console.log(not_mono_g)


const not_mono_g = JSON.stringify(GraphAnalyzer.treeID(new Graph('dag { A->B->C->D->E->F A<->C }'))) 
console.log(not_mono_g)
